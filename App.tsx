...
const GENRE_MAP: Record<string, string> = {
  "经典恐怖": "Classic Horror",
  "超级英雄动作": "Superhero Action",
  "黑暗科幻": "Dark Sci-Fi",
  "奇幻史诗": "High Fantasy",
  "霓虹侦探": "Neon Noir Detective",
  "废土末日": "Wasteland Apocalypse",
  "轻松喜剧": "Lighthearted Comedy",
  "青春校园 / 生活切片": "Teen Drama / Slice of Life",
  "自定义": "Custom"
};
...
const App: React.FC = () => {
  ...
  const generateBeat = async (history: ComicFace[], isRightPage: boolean, pageNum: number, isDecisionPage: boolean): Promise<Beat> => {
    ...
    const langName = LANGUAGES.find(l => l.code === selectedLanguage)?.name || "Chinese";
    ...
    const genreEnglish = GENRE_MAP[selectedGenre] || selectedGenre;

    // Determine Core Story Driver (Genre vs Custom Premise)
    let coreDriver = `GENRE: ${selectedGenre} (${genreEnglish}). TONE: ${storyTone}.`;
    if (selectedGenre === '自定义') {
    ...
    const isSliceOfLife = selectedGenre.includes("喜剧") || selectedGenre.includes("青春") || selectedGenre.includes("生活") || genreEnglish.includes("Comedy");

    // Guardrails to prevent everything becoming "Quantum Sci-Fi"
    const guardrails = `
    NEGATIVE CONSTRAINTS:
    1. UNLESS GENRE IS "Dark Sci-Fi" (黑暗科幻) OR "Superhero Action" (超级英雄动作) OR "Custom": DO NOT use technical jargon like "Quantum", "Timeline", "Portal", "Multiverse", or "Singularity".
    ...
    if (isFinalPage) {
        instruction += " FINAL PAGE. KARMIC CLIFFHANGER REQUIRED. You MUST explicitly reference the User's choice from PAGE 3 in the narrative and show how that specific philosophy led to this conclusion. Text must end with '未完待续...' (TO BE CONTINUED).";
...
`;
    try {
        ...
        if (isDecisionPage && !isFinalPage && (!parsed.choices || parsed.choices.length < 2)) parsed.choices = ["选项 A", "选项 B"];
  ...
  const generatePersona = async (desc: string): Promise<Persona> => {
      const genreEnglish = GENRE_MAP[selectedGenre] || selectedGenre;
      const style = selectedGenre === '自定义' ? "Modern American comic book art" : `${genreEnglish} comic`;
      try {
          const ai = getAI();
  ...
  const generateImage = async (beat: Beat, type: ComicFace['type']): Promise<string> => {
    ...
    const genreEnglish = GENRE_MAP[selectedGenre] || selectedGenre;
    const styleEra = selectedGenre === '自定义' ? "Modern American" : genreEnglish;
    let promptText = `STYLE: ${styleEra} comic book art, detailed ink, vibrant colors. `;
    
    if (type === 'cover') {
        const langName = LANGUAGES.find(l => l.code === selectedLanguage)?.name || "Chinese";
        promptText += `TYPE: Comic Book Cover. TITLE: "无限英雄" (OR LOCALIZED TRANSLATION IN ${langName.toUpperCase()}). Main visual: Dynamic action shot of [HERO] (Use REFERENCE 1).`;
    } else if (type === 'back_cover') {
        promptText += `TYPE: Comic Back Cover. FULL PAGE VERTICAL ART. Dramatic teaser. Text: "下一期再见" (NEXT ISSUE SOON).`;
    } else {
  ...
  const generateSinglePage = async (faceId: string, pageNum: number, type: ComicFace['type']) => {
      ...
      if (beat.focus_char === 'friend' && !friendRef.current && type === 'story') {
          try {
              const genreEnglish = GENRE_MAP[selectedGenre] || selectedGenre;
              const newSidekick = await generatePersona(selectedGenre === '自定义' ? "A fitting sidekick for this story" : `Sidekick for ${genreEnglish} story.`);
  ...
  const launchStory = async () => {
    ...
    if (selectedGenre === "青春校园 / 生活切片" || selectedGenre === "轻松喜剧") {
        availableTones = TONES.filter(t => t.includes("日常") || t.includes("治愈") || t.includes("幽默"));
    } else if (selectedGenre === "经典恐怖") {
        availableTones = TONES.filter(t => t.includes("内心独白") || t.includes("歌剧"));
    }
  ...
  const handleHeroUpload = async (file: File) => {
       try { const base64 = await fileToBase64(file); setHero({ base64, desc: "The Main Hero" }); } catch (e) { alert("主角上传失败"); }
  };
  const handleFriendUpload = async (file: File) => {
       try { const base64 = await fileToBase64(file); setFriend({ base64, desc: "The Sidekick/Rival" }); } catch (e) { alert("配角上传失败"); }
  };
